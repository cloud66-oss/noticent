...
module Noticent
  ...
  class Config
    ...
    class Builder
      ...
      def scope(name, klass: nil, payload_class: nil, constructor: nil, &block)
        scopes = @config.instance_variable_get(:@scopes) || {}

        raise BadConfiguration, "scope '#{name}' already defined" if scopes.include? name

        scope = Noticent::Definitions::Scope.new(@config, name, klass: klass, payload_class: payload_class, constructor: constructor)
